#!/usr/bin/env bash
# Sync skills from obra/superpowers (main) into this repo's .claude/skills/ (flattened).
# Run from repo root. lazy-code-skill's own skills are never touched.
# Skills-first behavior lives in .claude/rules/skill-first-engineering.mdc (not synced from upstream).
#
# Claude-adapted skills (not overwritten by sync; update by merging from upstream manually):
...
UPSTREAM_DIR="${REPO_ROOT}/.claude/.superpowers-upstream"
SKILLS_DIR="${REPO_ROOT}/.claude/skills"

# Skills to copy from Superpowers (Claude-adapted ones are skipped; see SKIP_SYNC below)
SUPERPOWERS_SKILLS=(
  brainstorming
...
# Claude-adapted: do not overwrite; port upstream changes manually when needed
...
for name in "${SUPERPOWERS_SKILLS[@]}"; do
  if is_skipped "$name"; then
    echo "Skip (Claude-adapted, merge manually): ${name}"
    continue
  fi
...
echo "Done. .claude/skills/ updated with Superpowers skills (flattened)."
echo "Claude-adapted skills (not synced): ${SKIP_SYNC[*]}"
