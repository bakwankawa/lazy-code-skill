...
| `.cursor/skills/vercel-labs/` | From vercel-labs/agent-skills: all skills under `skills/`; synced via sync-vercel-labs-skills.sh. |
...
| `.cursor/scripts/` | sync-superpowers-skills.sh (superpowers); sync-anthropic-skills.sh (anthropics/skills into .cursor/skills/anthropic/); sync-vercel-labs-skills.sh (vercel-labs/agent-skills into .cursor/skills/vercel-labs/); sync-all-upstreams.sh (runs all three sync scripts in sequence). |
...
2. Agent fetches INSTALL.md and runs: clone repo into `.cursor/lazy-code-skill`, copy all four skill folders (lazy-code-skill, superpowers, anthropic, vercel-labs), rules (replacing existing lazy-code-skill-*.mdc), and README (as `.cursor/lazy-code-skill-README.md`); copy project_architecture.md only if missing. Then remove the clone. Re-running install force-updates skills, rules, and README; **`.cursor/plans/` and `.cursor/project_architecture.md` are never overwritten** if they already exist.
