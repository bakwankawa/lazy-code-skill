...
1. **Access conversation history**: Cursor's API or context provides conversation messages
...
4. **Save to file**:
   ...
   - Ensure `.cursor/conversations/` directory exists (project-level)
...
Default: `.cursor/conversations/` (project-level)
...
- Scripts automatically find project root by locating `.cursor/` directory
...
- Integration with Cursor's native history
